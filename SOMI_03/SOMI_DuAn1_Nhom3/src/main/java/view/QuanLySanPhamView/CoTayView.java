...
import serviceimpl.QuanLySanPhamServiceImpl.ViewCoTayServiceImpl;
...
public class CoTayView extends javax.swing.JFrame {
    ...
    private ViewCoTayService service = new ViewCoTayServiceImpl();
    
    public CoTayView() {
        initComponents();
    }

    public void loadData(List<CoTay> chatLieu){
        defaultTableModel = (DefaultTableModel) tblCoTay.getModel();
        defaultTableModel.setRowCount(0);
        for(CoTay x: chatLieu){
            defaultTableModel.addRow(new Object[]{
                x.getId_co_tay(), x.getMa(), x.getTen()
            });
        }
    }
