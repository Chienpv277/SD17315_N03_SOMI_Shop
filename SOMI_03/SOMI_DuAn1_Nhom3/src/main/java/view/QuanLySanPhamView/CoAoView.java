...
import serviceimpl.QuanLySanPhamServiceImpl.ViewCoAoServiceImpl;
...
public class CoAoView extends javax.swing.JFrame {
    ...
    private ViewCoAoService service = new ViewCoAoServiceImpl();
    
    public CoAoView() {
        initComponents();
    }

    public void loadData(List<CoAo> chatLieu){
        defaultTableModel = (DefaultTableModel) tblCoAo.getModel();
        defaultTableModel.setRowCount(0);
        for(CoAo x: chatLieu){
            defaultTableModel.addRow(new Object[]{
                x.getId_co_ao(), x.getMa(), x.getTen()
            });
        }
    }
